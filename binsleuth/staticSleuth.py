...
class StaticAnalyses:
  ...
  def __init__(self, filename):
    ...
    self._project = angr.Project(filename, load_options={'auto_load_libs':False})
  ...
  def unmapped_memory(self):
    ...
    state = self._project.factory.entry_state(add_options={angr.options.STRICT_PAGE_ACCESS})
  ...
  def potential_hard_coded(self):
    ...
    for address in fun_addrs:
      ...
      if pg.found:
        potential = pg.found[0].state.posix.dumps(0)
        prompt = pg.found[0].state.posix.dumps(1)
        if len(potential): 
  ...
  def find_unconstrained(self):
    ...
    entry_state = self._project.factory.entry_state(add_options={so.REVERSE_MEMORY_NAME_MAP, so.TRACK_ACTION_HISTORY})
    
    sim = self._project.factory.simulation_manager(entry_state, save_unconstrained=True)
