...
class ProcSleuth:
    ...
    def _set_proc_state(self):
        ...
        try:
            self._proc_mem_map.extend(self._lock_proc.mem_maps())
        except:
            pass
    ...
    def monitor_processes(self, previous_process_list):
        ''' Print changes to process list to stdout
          ...
          work. Maybe you will have better luck
            ...
            if not matched:
                ...
                if name == self._exe:
                    ...
                    for c in proc_current.connections():
                        self._proc_con_memory[c] = {"start time": str(
                            datetime.fromtimestamp(proc_current.create_time()))}
    ...
    def get_exe(self, filename):
        return filename.split('\\')[-1] if '\\' in filename else filename.split('/')[-1] if '/' in filename else filename
    ...
    def _monitor_file_io(self):
        ...
        # so we exit
        try:
            cur_files = psutil.Process(self._lock_proc.pid).open_files()
        except:
            self._go = False
            return
        ...
        for new_file in cur_files:
            ...
            if not matched:
                ...
                # map file obj -> {}
                self._file_memory[new_file] = {
                    "start time": timestamp, "end time": "???"}
        ...
        for old_file in self._file_io:
            ...
            if not matched:
                timestamp = datetime.now()
                try:
                    self._file_memory[old_file]["end time"] = timestamp
                except:
                    self._file_memory[old_file]["end time"] = {
    ...
    def _monitor_network_cons(self):
        ...
        for old_con in self._proc_cons:
            ...
            if not matched:
                timestamp = datetime.now()
                try:
                    self._proc_con_memory[old_con]["end time"] = timestamp
                except:
                    self._proc_con_memory[old_con] = {
                        "start time": "???", "end time": timestamp}
            matched = 0
    ...
    def _monitor_children(self):
        ...
        try:
            child_state = [proc for proc in psutil.Process(
                self._lock_proc.pid).children(recursive=True)]
        except:
            self._go = False
            return
        ...
        for new_child in child_state:
            ...
            if not matched:
                ...
                self._proc_children_memory[new_child] = {"start time": timestamp, "end time": "???", "exe": new_child.exe(
                ), "open files": new_child.open_files(), "net cons": new_child.connections()}
            matched = 0
        matched = 0
        ...
        for old_child in self._proc_children:
            ...
            if not matched:
                timestamp = datetime.now()
                try:
                    self._proc_children_memory[old_child]["end time"] = timestamp
                except:
                    self._proc_children_memory[old_child] = {
                        "start time": "???", "end time": timestamp, "exe": "???", "open files": [], "net cons": []}
    ...
    def _d3js_con(self):
        ...
        for k, v in family_con_map.items():
            ...
            for con in v:
                try:
                    socket_con_map[k][str(con.laddr.ip) +
                                      ':' + str(con.laddr.port)].append(con)
                except:
                    socket_con_map[k][str(con.laddr.ip) +
                                      ':' + str(con.laddr.port)] = []
                    socket_con_map[k][str(con.laddr.ip) +
        ...
        for k, v in socket_con_map.items():
            ...
            final_con.append(
                {
                    "name": "Family: " + ("IPv4" if k == 2 else "IPv6" if k == 12 else str(k)) + "\tCount: " + str(len(outer_con)),
                    "children": outer_con
                }
            )
