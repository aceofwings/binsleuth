    ...
    def print_change(self, previous_process_list):
        ...
        for proc_current in current_process_list:
            ...
            if not matched:
                ...
                if name == self._exe:
                    print('\nHooked Process\n')
                    self._proc_con_memory[datetime.fromtimestamp(
                        proc_current.create_time())] = []
                    self._proc_con_memory[datetime.fromtimestamp(proc_current.create_time())].extend(
                        [(c, True) for c in proc_current.connections()])
    ...
    def graph_con_mem(self, outfile='net_connections', view=False):
        ...
        for k, v in self._proc_con_memory.items():
            ...
            for c in v:
                ...
                data = 'Src {}\tPort {} \n\nDest {}\tPort {} \n\nFamily {} \n\nStatus {}'.format(
                    con.laddr[0], con.laddr[1], con.raddr[0], con.raddr[1], str(con.family), con.status)
                digraph.edge(self.format_time(k), data,
        ...
        for i in range(1, len(self._proc_con_memory.keys())):
            ...
            digraph.edge(self.format_time(node1), self.format_time(
                node2), label=str(node2 - node1), color='blue')
    ...
    def graph_file_memory(self, outfile='file_operations', view=False):
        ...
        for k, v in self._file_memory.items():
            ...
            for f in v:
                ...
                digraph.attr('node', shape='circle')
                try:
                    data = '{}\nMode {}\nFlags{}'.format(str(file.path).replace(
                        ':', '[colon]'), str(file.mode), str(file.flags))
                except:
                    data = '{}'.format(str(file.path).replace(
                        ':', '[colon]').replace('\\', '/'))
                digraph.edge(self.format_time(k), data,
                             color='green' if is_new else 'red')
        ...
        for i in range(1, len(self._file_memory.keys())):
            ...
            digraph.edge(self.format_time(node1), self.format_time(
                node2), label=str(node2 - node1), color='purple')
